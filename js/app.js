...
(function(){
  ...
  function deleteProfile(id){
    ...
    [STORAGE_KEY, RACE_TIME_KEY, DAYORDER_KEY].forEach(base => {
  ...
  function renderPhaseCard(){
    ...
    if (throughDate){
      const elapsed = FLAT_DATES.filter(d => d <= throughDate);
  ...
  function renderWeekRail(){
    ...
    week.days.forEach(slot => {
      ...
      btn.className = `week-dot type-${content.sessionType} ${done?'done':''} ${partial?'partial':''} ${realDate===real?'is-today':''} ${realDate===viewDate?'is-viewing':''}`;
      if (partial) btn.style.setProperty('--pct', pct + '%');
      btn.innerHTML = `<span class="wd-letter">${slot.dayName.slice(0,1)}</span><span class="wd-ring">${fromISO(realDate).getDate()}</span>`;
      btn.setAttribute('aria-label', `${slot.dayName} ${fmtShort(realDate)}${done?' - complete':''}`);
  ...
  function renderFooter(){
    ...
    let streak = 0;
    if (cursor){
      if (!isDayComplete(cursor)) cursor = addDays(cursor, -1);
      while (cursor >= PROGRAM_START && isDayComplete(cursor)){
        streak++;
