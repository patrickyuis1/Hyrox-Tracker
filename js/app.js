...
(function(){
  ...
  function formatLogEntry(type, entry){
    if (!entry) return null;
    if (type === 'weight') return entry.weight ? `${entry.weight}kg` : null;
    if (type === 'time') return entry.time || null;
    if (type === 'distance_hr'){
      const parts = [];
      if (entry.distance) parts.push(`${entry.distance}km`);
      if (entry.hr) parts.push(`${entry.hr}bpm avg`);
  ...
  function logInputHTML(block, dateStr){
    if (!block.log) return '';
    const type = block.log.type;
    const prev = getMostRecentEntry(block.name, dateStr);
    const prevLabel = prev ? formatLogEntry(type, prev) : null;
    const prevText = prevLabel ? `Last: <b>${escapeHtml(prevLabel)}</b> (${fmtShort(prev.date)})` : 'No previous log yet';
    const current = getEntryOn(block.name, dateStr) || {};

    let inputs = '';
    if (type === 'weight'){
      inputs = `<input class="log-input" data-logfield="weight" type="text" inputmode="decimal" placeholder="kg" value="${escapeHtml(current.weight || '')}">`;
    } else if (type === 'time'){
      inputs = `<input class="log-input" data-logfield="time" type="text" inputmode="numeric" placeholder="mm:ss" value="${escapeHtml(current.time || '')}">`;
    } else if (type === 'distance_hr'){
      inputs = `<input class="log-input log-input-sm" data-logfield="distance" type="text" inputmode="decimal" placeholder="km" value="${escapeHtml(current.distance || '')}">
                <input class="log-input log-input-sm" data-logfield="hr" type="text" inputmode="numeric" placeholder="avg HR" value="${escapeHtml(current.hr || '')}">`;
    }
